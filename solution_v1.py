...
test_acc = np.zeros((len(test), 2), dtype=np.float64)
...
for fold in range(NFOLD):
    ...
    with torch.no_grad():
        ...
        test_acc += np.concatenate(parts).astype(np.float64)
    print("fold", fold, "complete", flush=True)

coef = test_acc / NFOLD
FLOOR = float(np.quantile(TR_CMAG, FLOOR_Q))
mag = np.linalg.norm(coef, axis=1, keepdims=True)
coef = coef * np.maximum(1.0, FLOOR / np.maximum(mag, 1e-9))
coef = coef * ALPHA
