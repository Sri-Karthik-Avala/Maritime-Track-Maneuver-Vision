...
for _sx in (1.0, -1.0):
    for _sy in (1.0, -1.0):
        _tab = make_feats(TE_VX * _sx, TE_VY * _sy,
                          0.5 + (TE_CX - 0.5) * _sx, 0.5 + (TE_CY - 0.5) * _sy, TE_ST)
        _mot = np.column_stack([TE_VX * _sx, TE_VY * _sy, np.ones(len(test))]).astype(np.float32)
        TTA_VIEWS.append((_sx, _sy, _tab, _mot))
  ...
  for fold in range(NFOLD):
      ...
      with torch.no_grad():
          ...
          for s in range(0, len(test), 64):
              sl = slice(s, s + 64)
              acc = 0.0
              for sx, sy, vtab, vmot in TTA_VIEWS:
                  im = XTE[sl]
                  if sx < 0:
                      im = im[:, :, ::-1]
                  if sy < 0:
                      im = im[:, ::-1]
                  o = net(to_tensor(im), torch.from_numpy(vtab[sl]),
                          torch.from_numpy(vmot[sl])).numpy()
                  o[:, 0] *= sx
                  o[:, 1] *= sy
                  acc = acc + o
              parts.append(acc / len(TTA_VIEWS))
