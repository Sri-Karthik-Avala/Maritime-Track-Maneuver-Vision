...
LOGMAG = np.log(TR_CMAG + 1e-6)
et_dir = np.zeros((len(test), 2), dtype=np.float64)
et_mag = np.zeros(len(test), dtype=np.float64)
nfit = 0
for fold in range(NFOLD):
    tri = np.where(fold_of != fold)[0]
    for s in ET_SEEDS:
        ...
        mm = ExtraTreesRegressor(n_estimators=ET_TREES, min_samples_leaf=ET_LEAF,
                                 max_features=0.5, n_jobs=NTHREAD, random_state=s)
        mm.fit(WTR[tri], LOGMAG[tri])
        et_mag += mm.predict(WTE)
        nfit += 1
    print("et fold", fold, "complete", flush=True)
et_dir /= nfit
et_mag = np.exp(et_mag / nfit)
...
magnitude = np.exp(MAG_W * np.log(np.maximum(et_mag, 1e-12)) + (1.0 - MAG_W) * np.log(MED_CMAG))
