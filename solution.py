...
GRAYW = torch.tensor([0.299, 0.587, 0.114]).view(1, 3, 1, 1)
...
def to_tensor_aug(arr):
    x = torch.from_numpy(np.ascontiguousarray(arr)).float().permute(0, 3, 1, 2) / 255.0
    x = x * (1.0 + (np.random.rand() - 0.5) * 2.0 * BRIGHT)
    m = x.mean(dim=(1, 2, 3), keepdim=True)
    x = (x - m) * (1.0 + (np.random.rand() - 0.5) * 2.0 * CONTRAST) + m
    g = (x * GRAYW).sum(1, keepdim=True)
    x = (x - g) * (1.0 + (np.random.rand() - 0.5) * 2.0 * SATUR) + g
    if np.random.rand() < GRAY_P:
        x = g.repeat(1, 3, 1, 1)
    return (x.clamp(0.0, 1.0) - IMNET_M) / IMNET_S
...
gpos = {u: i for i, u in enumerate(uniq)}
gid = np.array([gpos[g] for g in groups])
gcnt = np.bincount(gid, minlength=len(uniq)).astype(np.float64)
ROW_W = (1.0 / np.maximum(gcnt[gid], 1.0)) ** GROUP_BALANCE
...
for run in SEEDS:
  for fold in range(NFOLD):
      ...
      for ep in range(EPOCHS):
          ...
          wts = ROW_W[tr_idx]
          wts = wts / wts.sum()
          perm = np.random.choice(tr_idx, size=len(tr_idx), replace=True, p=wts)
          for s in range(0, len(perm), BATCH):
              ...
              opt.zero_grad()
              out = net(to_tensor_aug(im), torch.from_numpy(tb), torch.from_numpy(mb))
